...
export class ContentCleaner {
  ...
  clean(rawHtml: string, url = ''): CleanResult {
    ...
    try {
      ...
      const readableDocument = dom.window.document.cloneNode(true) as Document
      ...
      return {
        cleanedHtml,
        cleanedMarkdown,
        title: readable?.title?.trim() || extractTitle(html),
        author: readable?.byline?.trim() || extractAuthor(html)
      }
    } catch {
  ...
  private buildFallback(rawHtml: string, url: string): CleanResult {
...
function getFallbackHtml(rawHtml: string, url: string): string {
  ...
  return [
    '<article>',
    '<p>正文提取失败，请打开原文查看。</p>',
    url ? `<p><a href="${escapeHtml(url)}">查看原文</a></p>` : '',
    preview ? `<p>${preview}</p>` : '',
    '</article>'
  ].join('')
}
...
function extractTitle(html: string): string | undefined {
  ...
  const titleMatch = headingMatch ?? html.match(/<title[^>]*>(.*?)<\/title>/is)
  return titleMatch ? stripTags(titleMatch[1]) : undefined
}
