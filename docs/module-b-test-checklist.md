...
当前自动化已覆盖 B-SVC-01 和 B-SVC-02。B-SVC-03 到 B-SVC-05 建议在后续更完整的服务测试中继续补。
...
| B-UI-07 | 长文章 | 右侧正文区域可滚动 |
| B-UI-08 | 长代码块或表格 | 只在正文内部横向滚动，不撑破整个页面 |
| B-UI-09 | 窄屏宽度 | 按钮换行，正文不横向溢出 |
| B-UI-10 | 深色主题 | 背景、正文、按钮、代码块都切换到深色配色 |
